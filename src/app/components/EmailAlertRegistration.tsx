...
import { Mail, MapPin, Shield, CheckCircle, AlertTriangle, Info, Zap, AtSign, Bug, Send } from 'lucide-react'
    ...
    try {
      const response = await fetch('/api/test-sms', {
        ...
        body: JSON.stringify({
          ...
          impactLng: location?.lng || 77.2090,
      ...
      if (response.ok) {
        alert(`🚨 ${result.message}\n\n${demoMode ? 'Check browser console for demo messages.' : 'Check your Gmail inbox for emergency alert!'}`)
      } else {
  ...
  const testPersonalAlert = async () => {
    ...
    try {
      ...
      if (response.ok && result.success) {
        alert(`✅ ${result.message}\n\n${demoMode ? 'Check browser console for demo messages.' : 'Check your Gmail inbox for test email!'}`)
      } else {
...
DefendEarth System - Powered by Gmail SMTP`
  ...
  return (
    <motion.div 
      className="bg-gradient-to-br from-blue-900/40 to-purple-900/20 border border-blue-500/30 rounded-2xl p-6 backdrop-blur-sm"
    ...
    >
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          <h3 className="text-2xl font-bold text-blue-400">Gmail Emergency Alerts</h3>
          <p className="text-gray-300 text-sm">Powered by Nodemailer + Gmail SMTP - Beautiful HTML Emails</p>
        </div>
      </div>
      ...
      {demoMode && (
        ...
        >
          <div className="flex items-center gap-2 text-purple-300">
            ...
            <span className="font-semibold">Demo Mode - Gmail not configured</span>
          </div>
          <p className="text-purple-200 text-sm mt-1">
            Add Gmail credentials to send real beautiful HTML emails. Check console for simulated messages.
          </p>
        </motion.div>
      )}
        ...
        {!isRegistered ? (
          ...
          >
            ...
            {location && (
              ...
              >
                <div className="text-sm text-green-300 font-semibold">📍 Location Confirmed</div>
                <div className="text-xs text-gray-300">
                  Lat: {location.lat.toFixed(6)}, Lng: {location.lng.toFixed(6)}
                </div>
                <div className="text-xs text-green-400 mt-1">
                  You'll receive alerts for threats within 1000km of this location
                </div>
              </motion.div>
            )}
          ...
          >
            ...
            <div className="bg-gray-800/50 border border-gray-600 rounded-lg p-4 mb-4">
              <p className="text-gray-300 text-sm mb-2">
                <strong>🛡️ Gmail Protection Active!</strong> System monitors threats every 2 minutes.
              </p>
            ...
            <div className="mt-4 text-xs text-gray-400 space-y-1">
              <p><strong>Emergency Alert:</strong> Simulates asteroid threat with beautiful HTML email</p>
              <p><strong>Personal Alert:</strong> Tests system with professional HTML formatting</p>
              <p><strong>Direct Gmail Test:</strong> Tests raw Gmail SMTP functionality</p>
              <p><strong>Debug Status:</strong> Shows registered users and system info</p>
            </div>
            ...
            {debugInfo && (
              ...
              >
                <div className="text-xs text-gray-300">
                  <div className="font-semibold text-blue-400 mb-2">System Status:</div>
                  <div><strong>Total Users:</strong> {debugInfo.totalUsers || 0}</div>
                  <div><strong>Verified Users:</strong> {debugInfo.verifiedUsers || 0}</div>
                  <div><strong>Demo Mode:</strong> {debugInfo.demoMode ? 'Yes' : 'No'}</div>
                  <div><strong>Email Provider:</strong> {debugInfo.emailProvider || 'Unknown'}</div>
                  <div><strong>Email Available:</strong> {debugInfo.emailAvailable ? 'Yes' : 'No'}</div>
                  <div><strong>Monitoring:</strong> {debugInfo.monitoringActive ? 'Active' : 'Inactive'}</div>
                </div>
              </motion.div>
            )}
          </motion.div>

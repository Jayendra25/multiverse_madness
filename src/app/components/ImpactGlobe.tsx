...
let EffectComposer: any = ({ children }: { children: React.ReactNode }) => (
  <>{children}</>
);
let Bloom: any = () => null;
try {
  ...
  console.warn(
    "⚠️ @react-three/postprocessing not installed. Skipping bloom effects."
  );
}
...
function degToVec3(lat: number, lng: number, radius = 2) {
  ...
  useFrame((state) => {
    if (group.current) {
      ...
      group.current.rotation.x =
        Math.sin(state.clock.elapsedTime * 0.01) * 0.003;
    }
  });
...
function AnimatedAsteroid({
  ...
  const targetPos = useMemo(
    () => degToVec3(targetLat, targetLng, 2.02),
    [targetLat, targetLng]
  );
  ...
  useFrame((state) => {
    ...
    if (flameRef.current) {
      const intensity =
        0.6 + t * 2 + Math.sin(state.clock.elapsedTime * 20) * 0.15;
      const scale =
        intensity * (1 + Math.sin(state.clock.elapsedTime * 8) * 0.1);
      flameRef.current.scale.set(scale, scale, scale);
    }

    // Dynamic trail visualization
    if (trailRef.current && trajectoryPoints.length > 0) {
      const visiblePoints = trajectoryPoints.slice(
        0,
        Math.floor(eased * trajectoryPoints.length)
      );
    ...
    <div className="relative bg-black rounded-lg md:rounded-2xl overflow-hidden border border-gray-700 h-64 sm:h-80 md:h-96 lg:h-[500px] xl:h-[600px] w-full max-w-full">
      ...
      >
        <Suspense fallback={null}>
          ...
          <directionalLight
            position={[8, 4, 4]}
            intensity={textureMode === "night" ? 0.8 : 1.2}
          />
          <pointLight
            position={[-10, -5, -5]}
            intensity={textureMode === "night" ? 0.2 : 0.3}
          />

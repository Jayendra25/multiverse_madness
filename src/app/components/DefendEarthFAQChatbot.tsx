...
const faqData: FAQQuestion[] = [
  {
    ...
    answer:
      "Asteroids are rocky objects that orbit the Sun, primarily found in the asteroid belt between Mars and Jupiter. They become dangerous when their orbits bring them close to Earth. Even small asteroids can cause significant damage due to their incredible speed (up to 70 km/s). The DefendEarth system tracks over 50,000+ Near Earth Objects to identify potential threats.",
    followUp: [
      ...
      setMessages([
        {
          role: "assistant",
          content:
            "Hello! I'm ARIA, your DefendEarth assistant. I can answer questions about asteroids, planetary defense, and our protection systems. Click on any question below to learn more!",
          timestamp: new Date(),
          isWelcome: true,
        },
      ]);
    ...
    setMessages([
      {
        role: "assistant",
        content:
          "Chat reset! I'm ARIA, ready to help you learn about planetary defense. What would you like to know?",
        timestamp: new Date(),
        isWelcome: true,
      },
    ]);
  ...
  return (
    <>
      ...
      <motion.button
        ...
        className="fixed bottom-4 right-4 sm:bottom-6 sm:right-6 z-50 w-14 h-14 sm:w-16 sm:h-16 bg-gradient-to-r from-orange-500 to-red-600 rounded-full shadow-2xl flex items-center justify-center text-white hover:shadow-orange-500/25 transition-all duration-300"
      >
      ...
      <AnimatePresence>
        {isOpen && (
          <motion.div
            ...
            className="fixed inset-3 sm:inset-auto sm:bottom-24 sm:right-6 z-40 w-auto h-[calc(100vh-8rem)] sm:h-[600px] sm:w-96 max-w-full bg-gray-900/95 backdrop-blur-xl border border-gray-700 rounded-2xl shadow-2xl flex flex-col overflow-hidden"
          >
            {/* Header */}
            <div className="bg-gradient-to-r from-orange-500 to-red-600 p-3 sm:p-4 flex items-center justify-between">
            ...
            <div className="flex-1 overflow-y-auto p-3 sm:p-4 space-y-3 sm:space-y-4 scrollbar-thin scrollbar-thumb-gray-600 scrollbar-track-transparent">
              {messages.map((message, index) => (
                ...
                >
                  ...
                  <div
                    className={`max-w-[75%] xs:max-w-xs sm:max-w-md px-3 py-2 sm:px-4 sm:py-3 rounded-2xl ${
                      message.role === "user"
            ...
            {showQuestions && (
              <div className="p-3 sm:p-4 border-t border-gray-700 bg-gray-800/50">
                <p className="text-xs text-gray-400 mb-2 sm:mb-3 text-center">
                ...
                <div className="grid grid-cols-1 gap-1 sm:gap-2">
                  {currentQuestions.map((question, index) => (
                    <motion.button
                      ...
                      className="text-left p-2 sm:p-3 bg-gray-700 hover:bg-gray-600 rounded-lg text-xs sm:text-sm text-gray-200 hover:text-white transition-all duration-200 border border-gray-600 hover:border-gray-500"
                    >
                      💬 {question}
                    </motion.button>
                  ))}
                ...
                <motion.button
                  ...
                  className="w-full mt-2 sm:mt-3 p-2 bg-gradient-to-r from-orange-500/20 to-red-500/20 border border-orange-500/30 rounded-lg text-xs text-orange-300 hover:bg-orange-500/30 hover:text-orange-200 transition-all duration-200"
                >
            ...
            {/* Footer */}
            <div className="px-3 py-2 sm:px-4 sm:py-2 bg-gray-800/30 border-t border-gray-700">
              <p className="text-xs text-gray-500 text-center">
                🛡️ ARIA • DefendEarth Knowledge Base
              </p>
            </div>
          </motion.div>
        )}

...
export default function AIRiskDashboard() {
  ...
  const [currentView, setCurrentView] = useState<"scenarios" | "interactive">(
    "interactive"
  );
  ...
  const runRiskAnalysis = async () => {
    ...
    try {
      ...
      setError(
        "Failed to fetch real-time data. Using simulation data for demonstration."
      );
      ...
      <div className="max-w-7xl mx-auto p-6 bg-gradient-to-b from-gray-900 to-black text-white">
        <div className="flex flex-col items-center justify-center min-h-96">
          ...
          <p className="text-gray-400 text-center max-w-md">
            Fetching real-time NASA data and running machine learning
            predictions...
          </p>
        ...
        {scenarioInfo && currentView === "scenarios" && (
          <div className="mt-4 flex items-center justify-center space-x-4 text-sm">
            ...
            <div className="text-gray-400">
              Asteroid:{" "}
              <span className="text-white">{scenarioInfo.asteroidName}</span>
            </div>
            <div className="text-gray-400">
              Location:{" "}
              <span className="text-white">{scenarioInfo.locationName}</span>
            </div>
          </div>
        )}
          ...
          <div className="mb-8">
            <div className="flex flex-wrap justify-center gap-4">
              {[
                ...
                {
                  id: "ocean-impact",
                  label: "Ocean Impact (Pacific)",
                  icon: "🌊",
                },
                {
                  id: "rural-impact",
                  label: "Rural Impact (Plains)",
                  icon: "🌾",
                },
              ].map((scenario) => (
                <button
            ...
            <>
                  ...
                  <div className="flex items-center mb-6">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-400">
                        AI Confidence:{" "}
                        {(riskMetrics.casualties.confidence * 100).toFixed(1)}%
                      </p>
                    </div>
                  </div>
                ...
                >
                  <div className="flex items-center mb-6">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-400">
                        AI Confidence:{" "}
                        {(riskMetrics.economicImpact.confidence * 100).toFixed(
                          1
                        )}
                        %
                      </p>
                  ...
                  <div className="space-y-4">
                    ...
                    <div className="flex justify-between items-center">
                      ...
                      <span className="text-xl font-bold text-orange-400">
                        {riskMetrics.economicImpact.recoveryTime.toFixed(1)}{" "}
                        years
                      </span>
                    </div>
                  </div>
                  ...
                  {[
                    {
                      ...
                      probability:
                        riskMetrics.deflectionProbability.kineticImpactor,
                      color: "text-orange-400",
                    },
                    {
                      method: "Nuclear Deflection",
                      probability:
                        riskMetrics.deflectionProbability.nuclearDeflection,
                      color: "text-red-400",
                    },
                    {
                      method: "Gravity Tractor",
                      probability:
                        riskMetrics.deflectionProbability.gravityTractor,
                    ...
                    <div key={method.method} className="text-center">
                      ...
                      <div
                        className={`text-3xl font-bold ${method.color} mb-2`}
                      >
                      ...
                      <div className="w-full bg-gray-700 rounded-full h-2">
                        <div
                          className={`h-2 rounded-full bg-gradient-to-r ${
                            method.probability > 0.8
                              ? "from-green-500 to-green-400"
                              : method.probability > 0.6
              ...
              >
                ...
                <div className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
                  {[
                    {
                      factor: "Population Risk",
                      value: riskMetrics.riskFactors.populationRisk,
                      icon: Users,
                      color: "text-red-400",
                    },
                    {
                      factor: "Economic Risk",
                      value: riskMetrics.riskFactors.economicRisk,
                      icon: DollarSign,
                      color: "text-green-400",
                    },
                    {
                      factor: "Environmental Risk",
                      value: riskMetrics.riskFactors.environmentalRisk,
                      icon: Globe,
                      color: "text-blue-400",
                    },
                    ...(riskMetrics.riskFactors.tsunamiRisk
                      ? [
                          {
                            factor: "Tsunami Risk",
                            value: riskMetrics.riskFactors.tsunamiRisk,
                            icon: Waves,
                            color: "text-cyan-400",
                          },
                        ]
                    ...
                    <div key={risk.factor} className="text-center">
                      <risk.icon
                        className={`w-8 h-8 ${risk.color} mx-auto mb-2`}
                      />
                      <div className="text-sm text-gray-400 mb-2">
                        {risk.factor}
                      </div>
              ...
              >
                ...
                <div className="text-gray-300 leading-relaxed">
                  <p className="mb-2">
                    ...
                    {riskMetrics.deflectionProbability.nuclearDeflection >
                    riskMetrics.deflectionProbability.kineticImpactor
                      ? `Nuclear deflection shows highest success probability (${(
                          riskMetrics.deflectionProbability.nuclearDeflection *
                          100
                        ).toFixed(1)}%)`
                      : `Kinetic impactor shows highest success probability (${(
                          riskMetrics.deflectionProbability.kineticImpactor *
                          100
                        ).toFixed(1)}%)`}{" "}
                    for this scenario
                  ...
                  <p className="mb-2">
                    ...
                    {riskMetrics.riskFactors.populationRisk > 0.7
                      ...
                      : riskMetrics.riskFactors.tsunamiRisk &&
                        riskMetrics.riskFactors.tsunamiRisk > 0.5
                  ...
                  <p>
                    🕒 <strong>Time Sensitivity:</strong> Early detection and{" "}
                    {riskMetrics.deflectionProbability.leadTime}-year lead time
                    are crucial for mission success
                  </p>
                  {scenarioInfo && !scenarioInfo.isRealData && (
                    <p className="mt-4 text-orange-300 text-sm">
                      💡 <strong>Note:</strong> This analysis uses demonstration
                      data. With a NASA API key, real-time asteroid data would
                      be used for more accurate predictions.
                    </p>
                  )}
                </div>
              </motion.div>

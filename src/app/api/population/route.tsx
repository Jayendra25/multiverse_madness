...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    if (nominatimResponse.ok) {
      ...
      const populationData = estimatePopulationFromLocation(locationData, lat, lng);
...
function estimatePopulationFromLocation(locationData: any, lat: number, lng: number) {
  ...
  if (locationData.address) {
    ...
    const majorCities = ['new york', 'los angeles', 'chicago', 'london', 'tokyo', 'paris', 'berlin', 'sydney'];
...
function generateFallbackPopulationData(lat: number, lng: number) {
  ...
  const knownUrbanAreas = [
    { lat: 40.7, lng: -74.0, density: 10000, name: 'NYC Area' },
    { lat: 34.0, lng: -118.2, density: 3000, name: 'LA Area' },
    { lat: 51.5, lng: -0.1, density: 5500, name: 'London Area' },
    { lat: 35.7, lng: 139.7, density: 6200, name: 'Tokyo Area' }
  ];
  ...
  const nearestUrban = knownUrbanAreas.reduce((closest, area) => {
  ...
  if (nearestUrban.distance < 1) { // Within ~100km
